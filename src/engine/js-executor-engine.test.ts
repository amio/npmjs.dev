...
describe('JSExecutorEngine', () => {
  ...
  describe('Security and Performance Constraints', () => {
    test('should interrupt infinite loops', async () => {
      ...
      assert.ok(result.error.startsWith('InternalError') || result.error.includes('interrupted'), 
    ...
    test('should limit excessive memory allocation', async () => {
      ...
      assert.ok(result.error.startsWith('InternalError') && result.error.includes('out of memory'),
    ...
    test('should limit deep recursion', async () => {
      ...
      assert.equal(result.error, 'Execution error: Maximum call stack size exceeded',
